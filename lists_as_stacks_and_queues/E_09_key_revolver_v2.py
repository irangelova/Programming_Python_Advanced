...
while bullets and locks:
    ...
    while remaining_bullets > 0 and bullets and locks:  # Added bullets and locks checks
        current_lock = locks[0]
        current_bullet = bullets.pop()
        bullets_shot += 1

        if current_bullet >= current_lock:  # Fixed comparison (bullet should be >= lock)
            locks.popleft()
            print("Bang!")
        else:
            print("Ping!")
        ...
        if remaining_bullets == 0 and bullets:  # Moved reload check here
            print("Reloading!")

if locks:  # Simplified the end condition checks
